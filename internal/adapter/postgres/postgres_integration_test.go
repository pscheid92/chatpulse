...
func setupTestDB(t *testing.T) *pgxpool.Pool {
	...
	t.Cleanup(func() {
		ctx := context.Background()
		_, err := testPool.Exec(ctx, "TRUNCATE streamers, configs, eventsub_subscriptions CASCADE")
		if err != nil {
...
func TestRunMigrationsWithLock_Idempotency(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	err = RunMigrationsWithLock(ctx, pool)
	require.NoError(t, err)
...
func TestRunMigrationsWithLock_SchemaVerification(t *testing.T) {
	pool := setupTestDB(t)
	ctx := context.Background()

	// Verify streamers table exists with expected columns
	var exists bool
	...
	// Verify overlay_uuid column exists in streamers table
	err = pool.QueryRow(ctx, `
		SELECT EXISTS (
			SELECT FROM information_schema.columns
			WHERE table_name = 'streamers' AND column_name = 'overlay_uuid'
		)
	`).Scan(&exists)
	require.NoError(t, err)
	assert.True(t, exists)
