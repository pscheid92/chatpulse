...
func TestValidateConfig_EmptyTriggerFor(t *testing.T) {
	err := validateConfig("  ", "Right", "no", "Left", 30, "combined")
	assert.Error(t, err)
...
func TestValidateConfig_WhitespaceOnlyTrigger(t *testing.T) {
	...
	err := validateConfig(string(spaces), "Right", "no", "Left", 30, "combined")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "for trigger cannot be empty")
}

func TestValidateConfig_EmptyTriggerAgainst(t *testing.T) {
	err := validateConfig("yes", "Right", "  ", "Left", 30, "combined")
	assert.Error(t, err)
...
func TestValidateConfig_ForTriggerTooLong(t *testing.T) {
	...
	err := validateConfig(longTrigger, "Right", "no", "Left", 30, "combined")
	assert.Error(t, err)
...
func TestValidateConfig_AgainstTriggerTooLong(t *testing.T) {
	longTrigger := string(make([]byte, 501)) // 501 characters
	err := validateConfig("yes", "Right", longTrigger, "Left", 30, "combined")
	assert.Error(t, err)
...
func TestValidateConfig_AgainstLabelTooLong(t *testing.T) {
	...
	err := validateConfig("yes", "Right", "no", longLabel, 30, "combined")
	assert.Error(t, err)
...
func TestValidateConfig_InvalidDisplayMode(t *testing.T) {
	err := validateConfig("yes", "Right", "no", "Left", 30, "invalid")
	assert.Error(t, err)
...
func TestValidateConfig_Valid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateConfig(tt.triggerFor, tt.labelFor, tt.triggerAgainst, tt.labelAgainst, tt.memorySeconds, tt.displayMode)
